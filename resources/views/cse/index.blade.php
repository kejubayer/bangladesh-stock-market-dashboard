...
<div class="container-fluid">
    <h2 class="mb-2 text-center">CSE Live Market Data</h2>
    ...
    <div class="position-relative">
        ...
        <div class="table-responsive" style="overflow-x: auto; min-width: 100%;">
            <table class="table table-bordered table-hover table-striped w-100" id="cseTable">
...
<script>
    ...
    function fetchDseData() {
        ...
        $.getJSON('{{ url("/cse-data") }}', function(data) {
            let tbody = '';
            if (data.length) {
                data.forEach(function(stock, index) {
                    ...
                    tbody += `<tr>
                            ...
                            <td data-order="${parseFloat(stock.open.toString().replace(/,/g, ''))}">${stock.open}</td>
            ...
            if ($.fn.DataTable.isDataTable('#cseTable')) {
                dataTable.destroy();
            }

            $('#cseTable tbody').html(tbody);

            // Initialize DataTable
            dataTable = $('#cseTable').DataTable({
                pageLength: 15,
                lengthMenu: [
